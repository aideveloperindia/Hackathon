...
const createTransporter = () => {
  ...
  // Fallback: return null if no email config (don't throw - allow app to run without email)
  console.warn('⚠️ No email configuration found. Email features will be disabled.');
  console.warn('   To enable email, set GMAIL_USER and GMAIL_APP_PASSWORD or SMTP settings.');
  return null;
};
...
export async function sendEmail(options: EmailOptions): Promise<void> {
  ...
  if (!emailTransporter) {
    throw new Error('Email service not configured. Please set GMAIL_USER and GMAIL_APP_PASSWORD or SMTP settings.');
  }
